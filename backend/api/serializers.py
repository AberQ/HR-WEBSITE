...
class VacancySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            "created_by"
        ]  # Делает поле created_by доступным только для чтения

    def update(self, instance, validated_data):
        # Убираем 'created_by' из validated_data, если оно присутствует
        validated_data.pop(
            "created_by", None
        )  # Исключаем created_by из данных, чтобы не было ошибки
...
class VacancySerializerForCreateAPI(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        vacancy.employment_type = (
            employment_type  # Установите правильное поле для условий работы
        )
